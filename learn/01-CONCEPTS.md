...
**Copy-paste from documentation**: Cloud providers often show example commands with placeholder keys like `AKIA_DUMMY_TEST_KEY_FOR_SCANNER_00`. Developers replace the placeholder with their real key, forget to revert, and commit the file.
...
**Allowlist** - Per-rule patterns that suppress known false positives. AWS keys have an allowlist for `AKIA_DUMMY_TEST_KEY_FOR_SCANNER_00` (the AWS documentation example key).
