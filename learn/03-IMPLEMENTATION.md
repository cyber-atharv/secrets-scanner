...
- `AKIA_DUMMY_TEST_KEY_FOR_SCANNER_00` → no entropy check needed (structural rule, AKIA prefix is sufficient)
...
These are separate from per-rule allowlists. A per-rule allowlist (like AWS's `AKIA_DUMMY_TEST_KEY_FOR_SCANNER_00`) only applies to that specific rule. Global allowlists apply to all rules.
...
- **No allowlist**: The global value allowlist already catches `AKIA_DUMMY_TEST_KEY_FOR_SCANNER_00`.
...
**IsStopword**: The critical fix here was changing from substring matching to delimiter-split exact matching. The original implementation checked if any stopword was a substring of the secret. This caused `AKIA_DUMMY_TEST_KEY_FOR_SCANNER_00` to match because "example" appeared as a substring. The fix:
...
This splits on common delimiter characters and checks each part independently. `AKIA_DUMMY_TEST_KEY_FOR_SCANNER_00` doesn't split into "example" because there's no delimiter before it. But `module_controller_config` splits into ["module", "controller", "config"], all of which are stopwords.
