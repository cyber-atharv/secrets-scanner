...
import (
	...
	"github.com/cyber-atharv/portia/pkg/types"
)
...
func TestDetectAWSKey(t *testing.T) {
	...
	chunk := types.Chunk{ //nolint:gosec
		Content:   `aws_key = "AKIA_DUMMY_TEST_KEY_FOR_SCANNER_00"`,
		FilePath:  "config.py",
		LineStart: 1,
	}
	...
	assert.Equal(t, "AKIA_DUMMY_TEST_KEY_FOR_SCANNER_00", findings[0].Secret) //nolint:gosec
...
func TestDetectStripeKey(t *testing.T) {
	...
	chunk := types.Chunk{ //nolint:gosec
		Content:   `STRIPE_KEY=sk_test_DUMMY_STRIPE_KEY_00000000`,
		FilePath:  "env.sh",
		LineStart: 10,
	}
	...
	assert.Equal(t,
		"sk_test_DUMMY_STRIPE_KEY_00000000",
		findings[0].Secret,
	)
}
...
func TestDetectMultipleFindings(t *testing.T) {
	...
	chunk := types.Chunk{
		Content: `aws_key = "AKIA_DUMMY_TEST_KEY_FOR_SCANNER_00"` + "\n" +
			`stripe = sk_test_DUMMY_STRIPE_KEY_00000000`,
		FilePath:  "config.py",
		LineStart: 1,
	}
...
func TestDetectAllowedPathFiltered(t *testing.T) {
	...
	chunk := types.Chunk{ //nolint:gosec
		Content:   `AKIA_DUMMY_TEST_KEY_FOR_SCANNER_00`,
		FilePath:  "go.sum",
		LineStart: 1,
	}
...
func TestExtractSecret(t *testing.T) {
	...
	line := `key = "AKIA_DUMMY_TEST_KEY_FOR_SCANNER_00"` //nolint:gosec
	loc := []int{7, 27, 7, 27}
	got := extractSecret(line, loc, 1)
	assert.Equal(t, "AKIA_DUMMY_TEST_KEY_FOR_SCANNER_00", got)
